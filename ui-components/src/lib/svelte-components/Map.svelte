<script lang="ts">
  ...
  async function mount(node: HTMLElement): Promise<void> {
    ...
    if (polylines.length > 0 && markers.length > 0) {
      const segments = polylines
        .map((multiPolyline: string) => decodeMultiPolyline(multiPolyline))
        .flat();
      const polylineLayer = L.polyline(segments, {
      ...
      const segments = polylines
        .map((multiPolyline) => decodeMultiPolyline(multiPolyline))
        .flat();
      const polylineLayer = L.polyline(segments, {
        color: "cornflowerblue",
      }).addTo(map);
